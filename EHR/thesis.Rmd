...
```{r}
#load the data
lab_data <- readr::read_delim("data/10000-Patients (2)/LabsCorePopulatedTable.txt")
pat_data <- readr::read_delim("data/10000-Patients (2)/PatientCorePopulatedTable.txt")
diag_data <- readr::read_delim("data/10000-Patients (2)/AdmissionsDiagnosesCorePopulatedTable.txt")
adm_data <- readr::read_delim("data/10000-Patients (2)/AdmissionsCorePopulatedTable.txt")
...
hm.2 <- function(obj){
  try(
    heatmap.2(obj,Rowv=F,symm=T,col=colorRampPalette(c("brown","white","yellow"))(25),
              dendrogram='none'
              ,main = "Heatmap of Original EHR Data"
              ,trace='none'
              ,labRow = F
              ,labCol = F
...
ehr[which(var_types %in% "c")] %>% cor %>% hm.2 
```
...
# Compute total iterations (for progress later, if desired).
