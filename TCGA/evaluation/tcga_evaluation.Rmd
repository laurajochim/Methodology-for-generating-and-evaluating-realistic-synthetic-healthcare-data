...
compare_syn <- synthpop::compare(RRPA_synthpop$syn, RPPA_df)
compare_syn$plots
...
RRPA_synthpop$syn[,which(var_tcga %in% "c")] %>% cor %>% hm.2 
  

# Subset only continuous variables based on var_tcga
continuous_RPPA_syn <- RRPA_synthpop$syn[, which(var_tcga == "c")]
...
tcga_vc[,which(var_tcga %in% "c")] %>% cor %>% hm.2 
...
tcga_ctgan2[,which(var_tcga %in% "c")] %>% cor %>% hm.2 
...
synthpop_data_tcga <- RRPA_synthpop$syn
